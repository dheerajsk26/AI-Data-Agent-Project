...
class AgentSchema(BaseModel):
    # Nodes already accumulate messages manually (state.messages = state.messages + [...]) and
    # return the full state each time, so an Annotated[list, add] reducer here double-applies
    # the concatenation on every node that passes messages through unchanged.
    messages : list = Field(..., description="List of messages to be processed by the agent")
