...
def transform_load_tool(input_file_path:str,output_folder:str,output_format:str, user_question:str) -> str:
    ...
    prompt = f"""
            ...
            in a Python environment with Pandas installed. `pandas` is already imported as `pd`,
            so do not include any import statements.
            ...
            write the code to transform and save the data at {output_folder} in {output_format} format.
...
llm = pick_llm("medium")  # Sonnet 5 — good balance for tool-calling reliability
...
def llm_node(state:ETLAgentSchema):
    ...
    final_answer = llm_bind.invoke([system_prompt] + messages)
...
if __name__ == "__main__":
    ...
    response = etl_analyst.invoke(
         {"messages":[HumanMessage(content="""
            I want to transform the data stored in the 'data/extract/extracted_data.csv' file
            and save the transformed data in the 'data/transform' folder in the csv format.
