...
def curate_ques(state: AgentSchema) -> AgentSchema: 
    ...
    curation_prompt = f"""
    Rewrite the user's question below into a single, clear, well-formed question
    suitable for SQL generation against a database.

    Rules:
    - Output ONLY the rewritten question as plain text. One sentence.
    - Do NOT include markdown headers, bullet points, or code blocks.
    - Do NOT explain why the rewrite is better.
    - Do NOT add safety warnings, caveats, or alternatives — a separate safety
      check happens later in the pipeline, so just rewrite the question as asked.

    User's question: {user_question}
    """

    response = llm.invoke(curation_prompt).content
...
def generate_sql(state: AgentSchema) -> AgentSchema:
    ...
    # Strip markdown code fences the LLM sometimes wraps the SQL in (e.g. ```sql ... ```)
    generated_sql_query = re.sub(r"^```(?:sql)?\s*|\s*```$", "", generated_sql_query.strip(), flags=re.IGNORECASE)
...
if __name__ == "__main__":
    ...
    input_schema = {
        "messages": [],
        "user_question": "Find the top 5 customers who have spent the most money on orders in the last year, and provide their names and total spending amounts.",
        "curated_ques": "",
    ...
    # Execute the Graph
    DEBUG = False  # flip to True to see raw messages and full schema prompt

    sql_analyst_response = sql_analyst.invoke(input_schema)

    if DEBUG:
        print("Messages:", sql_analyst_response['messages'])
        print("Full prompt context:", sql_analyst_response['prompt_query_context'])
        print("--------------------------------")
    ...
    print("Safety Check:", "SAFE" if sql_analyst_response['is_safe'].lower() == "yes" else "UNSAFE")
    print("Reason:", sql_analyst_response['comments'])
    print()
